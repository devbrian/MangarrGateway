...
# Stealth browser binary (Patchright wraps Chromium; install it)
# Escalation fallback (only if Patchright fails Comix Cloudflare)
# uv add "camoufox[geoip]==0.4.*" && uv run camoufox fetch
# Dev
...
| **Patchright** | Camoufox 0.4.11 | Use Camoufox when Patchright (Chromium) stops passing Comix's Cloudflare. Camoufox (Firefox, C++ fingerprint spoofing) is the stronger stealth but heavier. Keep the browser behind an interface so this is a config flip. |
...
| **Plain Playwright (no stealth) for Cloudflare** | Current Cloudflare fingerprints and blocks vanilla Playwright/Chromium (`navigator.webdriver`, CDP leaks). It will work on MangaDex (antibot: none) but fail on Comix (`cloudflare+encrypted`). | Patchright (drop-in) primary; Camoufox escalation. |
